...
router.get('/loginTable/all', async (req, res) => {
  try {
    ...
    // Keep column order consistent with your existing paged route
    const cols = [
      'userID',
      'username',
      'password',
    ...
    if (wantCsv) {
      const csv = stringifyLoginCsv(rows);
      res.setHeader('Content-Type', 'text/csv; charset=utf-8');
      res.setHeader('Content-Disposition', 'attachment; filename="loginTable_all.csv"');
      return res.status(200).send(csv);
    }

    return res.json({ total: rows.length, rows });
  } catch (err) {
    console.error('GET /api/admin/loginTable/all error:', err?.message);
    return res.status(500).json({ error: 'server_error' });
  }
});
